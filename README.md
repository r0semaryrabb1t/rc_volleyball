...
底盘工作区的 `rc_usb_control_node` 将 STM32 CDC 帧内的右三档拨杆持续发布到
`/rc/strike_switch`，本工作区的 `strike_node` 按以下规则控制击球：

- 中档：首次启动或重连后进入并保持蓄力位，同时解除击球锁止
- 中档到上档：单次触发 `center`
- 中档到下档：单次触发 `strong`
- 上/下档返回中档不触发；保持上/下档不连续触发
- 非 `READY` 状态的击球边沿直接忽略，不排队
- RC 链路有效时，遥控器优先于键盘、Xbox、红外和视觉输入
- RC 超时后重新锁止，必须再次经过中档才能触发

拨杆字段及数值均可在两边 YAML 中调整。当前默认使用 `sw_right`，并按 DJI
约定使用 `UP=1`、`MID=3`、`DOWN=2`。正式部署的一键启动脚本必须让底盘和
机械臂进程使用相同的 `ROS_DOMAIN_ID`；当前底盘脚本默认值为 `88`，已有
`strike_pc` 脚本默认值为 `0`，不可直接混用默认值。
