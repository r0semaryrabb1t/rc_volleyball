...
bool UnitreeMotorNative::parseFeedback(const uint8_t* data, size_t len) {
  ...
  // 在缓冲区中搜索有效返回帧，避免前置噪声或多帧串联导致解析失败
  size_t frame_start = SIZE_MAX;
  for (size_t offset = 0; offset + 16 <= len; ++offset) {
    if (data[offset] != 0xFD || data[offset + 1] != 0xEE) {
      continue;
    }
    uint8_t fb_id = data[offset + 2] & 0x0F;
    if (fb_id != motor_id_) {
      continue;
    }
    uint16_t recv_crc = data[offset + 14] | (data[offset + 15] << 8);
    uint16_t calc_crc = calcCrcCcitt(data + offset, 14);
  ...
  memcpy(&torque_q8, &data[frame_start + 3], 2);
  memcpy(&speed_q7, &data[frame_start + 5], 2);
  memcpy(&pos_q15, &data[frame_start + 7], 4);
  memcpy(&temp, &data[frame_start + 11], 1);

  uint8_t merror = data[frame_start + 12] & 0x07;
